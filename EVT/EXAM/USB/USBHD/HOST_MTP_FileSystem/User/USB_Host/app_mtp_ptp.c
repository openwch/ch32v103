...
uint8_t MTP_SendData( uint8_t *pbuf, uint32_t len )
{
    ...
    while( len )
    {
        ...
        {
            R8_UH_TX_LEN = HostCtl[ index ].Interface[ 0 ].OutEndpSize[ 0 ];
            memcpy( USBHD_TX_Buf, pbuf, HostCtl[ index ].Interface[ 0 ].OutEndpSize[ 0 ] );
        }
        else
        {
            R8_UH_TX_LEN = len;
        ...
        s = USBHDH_Transact( ( USB_PID_OUT << 4 ) | HostCtl[ index ].Interface[ 0 ].OutEndpAddr[ 0 ],
                             HostCtl[ index ].Interface[ 0 ].OutEndpTog[ 0 ], 20000 );
        if( s == ERR_SUCCESS )
        {
            HostCtl[ index ].Interface[ 0 ].OutEndpTog[ 0 ] ^= RB_UH_T_TOG;
...
uint8_t MTP_RecvData( uint8_t *pbuf, uint32_t *plen )
{
    ...
    s = USBHDH_Transact( ( USB_PID_IN << 4 ) | HostCtl[ index ].Interface[ 0 ].InEndpAddr[ 0 ],
                         HostCtl[ index ].Interface[ 0 ].InEndpTog[ 0 ], 20000 );
    if( s == ERR_SUCCESS )
    {
        HostCtl[ index ].Interface[ 0 ].InEndpTog[ 0 ] ^= RB_UH_R_TOG;
...
void USBH_MainDeal( void )
{
    ...
    if( s == ROOT_DEV_CONNECTED )
    {
        ...
        RootHubDev.DeviceIndex = DEF_USBHD_PORT_INDEX * DEF_ONE_USB_SUP_DEV_TOTAL;
