...
#if ( SystemCoreClock == SYSCLK_FREQ_72MHz ) 
    RCC_USBCLKConfig( RCC_USBCLKSource_PLLCLK_1Div5 );
#elif ( SystemCoreClock == SYSCLK_FREQ_48MHz ) 
...
uint8_t USBHDH_CtrlTransfer( uint8_t ep0_size, uint8_t *pbuf, uint16_t *plen )
{
    ...
    Delay_Us( 100 );
    s = USBHDH_Transact( ( R8_UH_TX_LEN ? USB_PID_IN << 4 | 0x00: USB_PID_OUT << 4 | 0x00 ), RB_UH_R_TOG | RB_UH_T_TOG, DEF_CTRL_TRANS_TIMEOVER_CNT );  /* STATUS½×¶Î */
    if( s != ERR_SUCCESS )
    {
        return s;
    }
