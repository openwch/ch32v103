...
void USART2_Receive_Handle( void )
{
    ...
    else if( flag == 1 )
    {
        ...
        status = USBHD_Endp_DataUp( DEF_UEP1, KB_Data_Pack, sizeof( KB_Data_Pack ), DEF_UEP_CPY_LOAD );
...
void MS_Scan_Handle( void )
{
    ...
    if( flag )
    {
        ...
        status = USBHD_Endp_DataUp( DEF_UEP2, MS_Data_Pack, sizeof( MS_Data_Pack ), DEF_UEP_CPY_LOAD );
