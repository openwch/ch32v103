...
void UART2_Rx_Service( void )
{
    ...
    if (UART2_Rx_RemainLen)
    {
        ...
        if (USBHD_Endp_Busy[DEF_UEP2] == 0x00)
        {
            ...
            if (pkg_len)
            {
                ...
                USBHD_EP2_Buf[0] = pkg_len;
                memcpy(USBHD_EP2_Buf + 1,&UART2_RxBuffer[UART2_Rx_Deal_Ptr],pkg_len);
                R16_UEP2_T_LEN = pkg_len + 1;
                USBHD_Endp_Busy[DEF_UEP2] = 1;
                R8_UEP2_CTRL = (R8_UEP2_CTRL & ~MASK_UEP_T_RES) | UEP_T_RES_ACK;            // Start Upload
...
void UART2_Tx_Service( void )
{
    ...
    if (UART2_Tx_Flag)
    {
        if (USART_GetFlagStatus(USART2, USART_FLAG_TC))                                  // Check whether uart2 has finished sending.
        {
            ...
            UART2_Tx_Flag = 0;
            NVIC_DisableIRQ(USBHD_IRQn);                                                  // Disable USB interrupts
