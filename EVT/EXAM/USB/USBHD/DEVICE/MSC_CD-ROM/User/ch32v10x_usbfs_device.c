...
void USBHD_Device_Endp_Init(void)
{
    ...
    R16_UEP2_DMA = (uint16_t)(uint32_t)CDROM_In_Buf;
    R16_UEP3_DMA = (uint16_t)(uint32_t)CDROM_Out_Buf;
        ...
        switch ( intst & MASK_UIS_TOKEN )
        {
            ...
            case UIS_TOKEN_SETUP:
                ...
                if ( ( USBHD_SetupReqType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )
                {
                    ...
                    {
                        if (USBHD_SetupReqCode == CMD_CDROM_GET_MAX_LUN)
                        {
                            USBHD_EP0_Buf[0] = 0;
                            pUSBHD_Descr = (uint8_t*)USBHD_EP0_Buf;
                            len = 1;
                        }
                        else if (USBHD_SetupReqCode == CMD_CDROM_RESET)
                        {
                            /* CDROM Reset */
