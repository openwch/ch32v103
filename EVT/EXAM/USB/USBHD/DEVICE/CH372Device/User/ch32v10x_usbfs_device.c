...
__attribute__ ((aligned(4))) uint8_t USBHD_EP3_Buf[ DEF_USBD_EP3_FS_SIZE*2 ];  //ep3_out(64)+ep3_in(64)
__attribute__ ((aligned(4))) uint8_t USBHD_EP4_Buf[ DEF_USBD_EP4_FS_SIZE*2 ];  //ep4_out(64)+ep4_in(64)
__attribute__ ((aligned(4))) uint8_t USBHD_EP5_Buf[ DEF_USBD_EP5_FS_SIZE*2 ];  //ep5_out(64)+ep5_in(64)
__attribute__ ((aligned(4))) uint8_t USBHD_EP6_Buf[ DEF_USBD_EP6_FS_SIZE*2 ];  //ep6_out(64)+ep6_in(64)
...
void USBHD_Device_Endp_Init(void)
{
    ...
    R8_UEP5_6_MOD = RB_UEP6_TX_EN | RB_UEP5_RX_EN;
    ...
    R16_UEP4_DMA = (uint16_t)(uint32_t)USBHD_EP4_Buf;

    R16_UEP5_DMA = (uint16_t)(uint32_t)USBHD_EP5_Buf;
    R16_UEP6_DMA = (uint16_t)(uint32_t)USBHD_EP6_Buf;
                ...
                switch ( intst & ( MASK_UIS_TOKEN | MASK_UIS_ENDP ) )
                {
                    ...
                    case UIS_TOKEN_OUT | DEF_UEP3:
                        if ( intst & RB_UIS_TOG_OK )
                        {
                            ...
                            for(i=0; i<len; i++)
                            {
                                USBHD_EP4_Buf[ i ] = ~USBHD_EP3_Buf[ i ];
                            }
                            R16_UEP4_T_LEN = len;
                    ...
                    case UIS_TOKEN_OUT | DEF_UEP5:
                        if ( intst & RB_UIS_TOG_OK )
                        {
                            len = (uint16_t)(R16_USB_RX_LEN&MASK_UIS_RX_LEN);
                            R8_UEP5_CTRL ^= RB_UEP_R_TOG;
                            R8_UEP5_CTRL = (R8_UEP5_CTRL & ~MASK_UEP_R_RES) | UEP_R_RES_NAK;
                            for(i=0; i<len; i++)
                            {
                                USBHD_EP6_Buf[ i ] = ~USBHD_EP5_Buf[ i ];
                            }
                            R16_UEP6_T_LEN = len;
                        ...
                        case USB_GET_STATUS:
                            ...
                            else if( ( USBHD_SetupReqType & USB_REQ_RECIP_MASK ) == USB_REQ_RECIP_ENDP )
                            {
                                switch( (uint8_t)( USBHD_SetupReqIndex & 0xFF ) )
                                {
                                    ...
                                    case ( DEF_UEP_OUT | DEF_UEP5 ):
                                        if( ( R8_UEP5_CTRL & MASK_UEP_R_RES ) == UEP_R_RES_STALL )
                                        {
                                            USBHD_EP0_Buf[ 0 ] = 0x01;
                                        }
                                        break;
                                    ...
                                    case ( DEF_UEP_IN | DEF_UEP6 ):
                                        if( ( R8_UEP6_CTRL & MASK_UEP_T_RES ) == UEP_T_RES_STALL )
                                        {
                                            USBHD_EP0_Buf[ 0 ] = 0x01;
                                        }
                                        break;
