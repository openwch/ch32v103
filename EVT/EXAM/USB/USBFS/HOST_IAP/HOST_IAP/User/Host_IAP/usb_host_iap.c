...
ENUM_START:
    ...
    s = USBFSH_GetConfigDescr( RootHubDev.bEp0MaxPks, Com_Buffer, DEF_COM_BUF_LEN, &len );
    if( s == ERR_SUCCESS )
    {
...
void IAP_Main_Deal( void )
{
    ...
    if( (CHRV3DiskStatus >= DISK_MOUNTED) && op_flag )
    {
        ...
        strcpy( (PCHAR)mCmdParam.Open.mPathName, DEF_IAP_FILE_NAME );
        /* open file */
        ret = CHRV3FileOpen( );
        ...
        if ( ret == ERR_MISS_DIR || ret == ERR_MISS_FILE )
        {
            ...
            {
                strcpy( (PCHAR)mCmdParam.Open.mPathName, (PCHAR)pCodeStr );       //Search for filenames, * is a wildcard, for all files or subdirectories
                ret = strlen( (PCHAR)mCmdParam.Open.mPathName );
