 ...
 * @fn      TIM3_Init
 *
 * @brief   Initialize timer3 for getting keyboard and mouse.
 *
...
ENUM_START:
    ...
    s = USBHDH_GetConfigDescr( RootHubDev.bEp0MaxPks, Com_Buf, DEF_COM_BUF_LEN, &len );
    if( s == ERR_SUCCESS )
    {
                        ...
                        else
                        {
                            ...
                            if( Com_Buf[ i + 1 ] == DEF_DECR_ENDPOINT )
                            {
                                ...
                                if( ( (PUSB_ENDP_DESCR)( &Com_Buf[ i ] ) )->bEndpointAddress & 0x80 )
                                {
                                    ...
                                    HostCtl[index].Interface[ num ].InEndpSize[ innum ] = ( (PUSB_ENDP_DESCR)( &Com_Buf[ i ] ) )->wMaxPacketSize;
                                ...
                                else
                                {
                                    ...
                                    HostCtl[index].Interface[ num ].OutEndpSize[ outnum ] = ( (PUSB_ENDP_DESCR)( &Com_Buf[ i ] ) )->wMaxPacketSize;
...
/*********************************************************************
 * @fn      KM_Deal_HidReportDesc
 *
 * @brief   Get keyboard and mouse report descriptor.
 *
 * @para    index: USB device number.
 *
 * @return  none
...
uint8_t USBH_EnumHidDevice( uint8_t index, uint8_t ep0_size )
{
    ...
    if( Com_Buf[ 6 ] )
    {
        ...
        s = USBHDH_GetStrDescr( ep0_size, Com_Buf[ 6 ], Com_Buf );
        if( s == ERR_SUCCESS )
        {
    ...
    if( DevDesc_Buf[ 14 ] )
    {
        ...
        s = USBHDH_GetStrDescr( ep0_size, DevDesc_Buf[ 14 ], Com_Buf );
        if( s == ERR_SUCCESS )
        {
    ...
    if( DevDesc_Buf[ 15 ] )
    {
        ...
        s = USBHDH_GetStrDescr( ep0_size, DevDesc_Buf[ 15 ], Com_Buf );
        if( s == ERR_SUCCESS )
        {
    ...
    if( DevDesc_Buf[ 16 ] )
    {
        ...
        s = USBHDH_GetStrDescr( ep0_size, DevDesc_Buf[ 16 ], Com_Buf );
        if( s == ERR_SUCCESS )
        {
                    ...
                    else
                    {
                        ...
                        if( Com_Buf[ i + 1 ] == DEF_DECR_ENDPOINT )
                        {
                            ...
                            if( ( (PUSB_ENDP_DESCR)( &Com_Buf[ i ] ) )->bEndpointAddress & 0x80 )
                            {
                                ...
                                HostCtl[ index ].Interface[ 0 ].InEndpSize[ 0 ] = ( (PUSB_ENDP_DESCR)( &Com_Buf[ i ] ) )->wMaxPacketSize;
...
uint8_t USBH_EnumHubDevice( void )
{
    ...
    {
        DUG_PRINTF("Get StringDesc4: ");
        s = USBHDH_GetStrDescr( RootHubDev.bEp0MaxPks, Com_Buf[ 6 ], Com_Buf );
        if( s == ERR_SUCCESS )
        {
            /* Print the string descriptor contained in the configuration descriptor */
#if DEF_DEBUG_PRINTF
    ...
    {
        DUG_PRINTF("Get StringDesc1: ");
        s = USBHDH_GetStrDescr( RootHubDev.bEp0MaxPks, DevDesc_Buf[ 14 ], Com_Buf );
        if( s == ERR_SUCCESS )
        {
            /* Print USB vendor string descriptor */
#if DEF_DEBUG_PRINTF
    ...
    {
        DUG_PRINTF("Get StringDesc2: ");
        s = USBHDH_GetStrDescr( RootHubDev.bEp0MaxPks, DevDesc_Buf[ 15 ], Com_Buf );
        if( s == ERR_SUCCESS )
        {
            /* Print USB product string descriptor */
#if DEF_DEBUG_PRINTF
    ...
    {
        DUG_PRINTF("Get StringDesc3: ");
        s = USBHDH_GetStrDescr( RootHubDev.bEp0MaxPks, DevDesc_Buf[ 16 ], Com_Buf );
        if( s == ERR_SUCCESS )
        {
            /* Print USB serial number string descriptor */
#if DEF_DEBUG_PRINTF
...
uint8_t USBH_EnumHubPortDevice( uint8_t hub_port, uint8_t *paddr, uint8_t *ptype )
{
    ...
    {
        enum_cnt++;
        s = USBHDH_GetDeviceDescr( &RootHubDev.Device[ hub_port ].bEp0MaxPks, DevDesc_Buf );
        if( s == ERR_SUCCESS )
        {
#if DEF_DEBUG_PRINTF
            for( i = 0; i < 18; i++ )
    ...
    {
        enum_cnt++;
        s = USBHDH_SetUsbAddress( RootHubDev.Device[ hub_port ].bEp0MaxPks, \
    ...
    {
        enum_cnt++;
        s = USBHDH_GetConfigDescr( RootHubDev.Device[ hub_port ].bEp0MaxPks, Com_Buf, DEF_COM_BUF_LEN, &len );
        if( s == ERR_SUCCESS )
        {
#if DEF_DEBUG_PRINTF
            for( i = 0; i < len; i++ )
    ...
    {
        enum_cnt++;
        s = USBHDH_SetUsbConfig( RootHubDev.Device[ hub_port ].bEp0MaxPks, cfg_val );
        if( s != ERR_SUCCESS )
        {
            DUG_PRINTF( "Err(%02x)\r\n", s );
            if( enum_cnt >= 10 )
...
uint8_t KB_SetReport( uint8_t index, uint8_t ep0_size, uint8_t intf_num )
{
    ...
    {
        s = USBHDH_SendEndpData( HostCtl[ index ].Interface[ intf_num ].OutEndpAddr[ 0 ],
...
void USBH_MainDeal( void )
{
    ...
    s = USBHDH_CheckRootHubPortStatus( RootHubDev.bStatus ); // Check USB device connection or disconnection
    if( s == ROOT_DEV_CONNECTED )
    {
        DUG_PRINTF("USB Dev In.\r\n");
        
        RootHubDev.bStatus = ROOT_DEV_CONNECTED; // Set connection status
        RootHubDev.DeviceIndex = DEF_USBHD_PORT_INDEX;
    ...
    if( RootHubDev.bStatus >= ROOT_DEV_SUCCESS )
    {
            ...
            for( intf_num = 0; intf_num < HostCtl[ index ].InterfaceNum; intf_num++ )
            {
                for( in_num = 0; in_num < HostCtl[ index ].Interface[ intf_num ].InEndpNum; in_num++ )
                {
                    ...
                    if( HostCtl[ index ].Interface[ intf_num ].InEndpTimeCount[ in_num ] >= HostCtl[ index ].Interface[ intf_num ].InEndpInterval[ in_num ] )
                    {
                        ...
                        /* Get endpoint data */
                        s = USBHDH_GetEndpData( HostCtl[ index ].Interface[ intf_num ].InEndpAddr[ in_num ], 
                        ...
                        else if( s == ( USB_PID_STALL | ERR_USB_TRANSFER ) )
                        {
                            ...
                            /* Clear endpoint */
                            USBHDH_ClearEndpStall( RootHubDev.bEp0MaxPks, HostCtl[ index ].Interface[ intf_num ].InEndpAddr[ in_num ] | 0x80 );
        ...
        else if( RootHubDev.bType == USB_DEV_CLASS_HUB )
        {
            ...
            if( HostCtl[ index ].Interface[ 0 ].InEndpTimeCount[ 0 ] >= HostCtl[ index ].Interface[ 0 ].InEndpInterval[ 0 ] )
            {
                ...
                /* Select HUB port */
                USBHDH_SetSelfAddr( RootHubDev.bAddress );
                USBHDH_SetSelfSpeed( RootHubDev.bSpeed );

                /* Get HUB interrupt endpoint data */
                s = USBHDH_GetEndpData( HostCtl[ index ].Interface[ 0 ].InEndpAddr[ 0 ], &HostCtl[ index ].Interface[ 0 ].InEndpTog[ 0 ], Com_Buf, &len );
                if( s == ERR_SUCCESS )
                {
                    HostCtl[ index ].Interface[ 0 ].InEndpTog[ 0 ] ^= RB_UEP_R_TOG;
                    ...
                    for( hub_port = 0; hub_port < RootHubDev.bPortNum; hub_port++ )
                    {
                        ...
                        if( s == ERR_USB_CONNECT )
                        {
                            ...
                            RootHubDev.Device[ hub_port ].DeviceIndex = hub_port + 1;
                        }
                        else
                        {
                            hub_dat &= ~( 1 << ( hub_port + 1 ) );
                        ...
                        if( RootHubDev.Device[ hub_port ].bStatus == ROOT_DEV_CONNECTED )
                        {
                            ...
                            USBHDH_SetSelfAddr( RootHubDev.Device[ hub_port ].bAddress );
                            USBHDH_SetSelfSpeed( RootHubDev.Device[ hub_port ].bSpeed );
                            ...
                            if( s == ERR_SUCCESS )
                            {
                                if( RootHubDev.Device[ hub_port ].bType == USB_DEV_CLASS_HID )
                                {
                                    ...
                                    s = USBH_EnumHidDevice( RootHubDev.Device[ hub_port ].DeviceIndex, RootHubDev.Device[ hub_port ].bEp0MaxPks );
                                    if( s == ERR_SUCCESS )
                                    {
